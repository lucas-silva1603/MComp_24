clear all
%--------------------------------------------------------------------------
%	Tarefa 22 : Introduzir condicoes mistas: essenciais homogeneas em
%		    1 e 4 + condicoes de Neumann, gama=1 em 6 e 5
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
%	Tarefas 22 : adicionar aqui condicoes de fronteira naturais de Neumann
gama = 1
fg(5) = fg(5) + gama/2
fg(6) = fg(6) + gama/2
%--------------------------------------------------------------------------
...
%----------------- Tarefa 22 tambem aqui ----------------------------------
boom = 1.0e+16
for i=1:Nnds
    if (i ~= 3) %   exclui o no interior
    %   Tarefa 22: impor condicoes de fronteira essenciais aqui
        if (i==1)
        Kr (i,i) = boom
        fr(i) = boom *0		% o valor pretendido aqui e zero
        end
        if (i==4)
        Kr (i,i) = boom
        fr(i) = boom *0		% o valor pretendido aqui e zero
